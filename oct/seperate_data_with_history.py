...
# === 1️⃣ Read main CSV ===
input_file = "data-oct.csv"  # change to your CSV filename
df = pd.read_csv(input_file)

# === 2️⃣ Define patient info columns ===
patient_info = [
...
# === 3️⃣ Define data columns by year ===
groups = {
    "birth": [
...
# === 4️⃣ Export cumulative files ===
output_folder = "split_data_with_previous_years"
os.makedirs(output_folder, exist_ok=True)

cumulative_cols = patient_info.copy()  # start with patient info

for name, cols in groups.items():
    # Add current year's columns to cumulative list
    cumulative_cols.extend([c for c in cols if c not in cumulative_cols])

    # Only include columns that exist in dataframe
    selected_cols = [c for c in cumulative_cols if c in df.columns]
    ...
    print(f"✅ Saved {name}.csv with {len(selected_cols)} columns (includes previous years).")

print("🎉 All cumulative year-based CSV files exported successfully!")
