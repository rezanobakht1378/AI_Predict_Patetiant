...
# === 1️⃣ Read the main CSV ===
input_file = "data-oct.csv"  # Change to your filename
...
# === 3️⃣ Export each group to a CSV ===
output_folder = "split_data"  # Folder to store outputs
import os
...
for name, cols in groups.items():
    ...
    sub_df.to_csv(f"{output_folder}/{name}.csv", index=False)
    print(f"✅ Saved {name}.csv with {len(existing_cols)} columns.")
